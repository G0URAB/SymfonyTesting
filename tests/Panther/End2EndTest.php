...
class End2EndTest extends PantherTestCase
{
    ...
    {
        $user = $this->entityManager->getRepository(User::class)->findOneBy(['email'=>"grv_sh@yahoo.co.in"]);
        ...
        $this->entityManager->getRepository(User::class)->clear();
        $user = $this->entityManager->getRepository(User::class)->findOneBy(['email'=>"grv_sh@yahoo.co.in"]);
        $newPicture=$user->getProfilePicture();
        $this->assertFalse($oldPicture==$newPicture);

        //3rd test: new file's existence in expected folder
        $this->assertTrue(file_exists($this->projectDirectory."/public/images/profile/".$newPicture));
